$("#save").click(function(){
    ...
    if (username!="" && password!="") {
        $.ajax({
            ...
            url: "http://localhost:8080/Tester/protal/login.do",//url
            data: JSON.stringify({
                username: username,
                password: password
            }),
            success: function (result) {
                if (result.code == 0) {
                    ...
                    window.location.href="testerIndex.html";
                } else {
                    alert("用户名或密码错误！");
                }
            }
...
$("#save2").click(function(){
    ...
    if (username!="" && password!="") {
        $.ajax({
            ...
            dataType: "json",//预期服务器返回的数据类型
            contentType: "application/json",
            url: "http://localhost:8080/Amaldar/protal/login.do",//url
            data: JSON.stringify({
                username: username,
                password: password
            }),
...
$("#save3").click(function(){
    ...
    if (username!="" && password!="") {
        $.ajax({
            ...
            dataType: "json",//预期服务器返回的数据类型
            contentType: "application/json",
            url: "http://localhost:8080/Developer/protal/login.do",//url
            data: JSON.stringify({
                username: username,
                password: password
            }),
            success: function (result) {
                if (result.code == 0) {
                    ...
                    window.location.href="developerIndex.html";
                } else {
                    alert("用户名或密码错误！");
                }
            }
