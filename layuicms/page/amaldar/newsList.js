layui.config({
	...
	var form = layui.form(),
        ...
        //加载页面数据
        var newsData = '';
        $.get("http://localhost:8082/Amaldar/Manager/getAll.do", function (result) {
    ...
    $(".search_btn").click(function(){
        if($(".search_input").val() != ''){
            ...
            setTimeout(function(){
                $.ajax({
                    url : "http://localhost:8082/Amaldar/Manager/selectLikeUsername.do",
                    type : "get",
                    dataType : "json",
		...
		if($checkbox.is(":checked")){
			...
			layer.confirm("确定删除选中这些用户"+names+"?",{icon:3, title:'提示信息'},function(index){
	            ...
	            setTimeout(function(){
	            	...
	            	$.ajax({
                        url : "http://localhost:8082/Amaldar/Manager/deleteSome.do?ids="+ids,
                        type : "DELETE",
		...
		layer.confirm("确定删除用户"+name+"?",{icon:3, title:'提示信息'},function(index){
			...
			for(var i=0;i<newsData.length;i++){
				if(newsData[i].id == _this.attr("data-id")){
				    $.ajax({
                        url : "http://localhost:8082/Amaldar/Manager/delete.do?id="+newsData[i].id+"",
                        type : "post",
                        dataType: "json",//预期服务器返回的数据类型
                        success: function (result) {
